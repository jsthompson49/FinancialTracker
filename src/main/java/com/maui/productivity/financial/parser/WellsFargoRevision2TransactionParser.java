...
@Log4j2
public class WellsFargoRevision2TransactionParser implements TransactionParser {
    ...
    private static final int CHECK_NUMBER_COLUMN_INDEX = 3;
    private static final int MINIMUM_COLUMN_COUNT = 4;
    ...
    public List<Transaction> parse(final String data) throws IOException  {
        ...
        for (final CSVRecord record : parser) {
            ...
            final String checkNumber = record.get(CHECK_NUMBER_COLUMN_INDEX);
            transactions.add(new Transaction(amount, date, getTransactionDescription(description, checkNumber)));
        }

        return transactions;
    }

    private String getTransactionDescription(final String description, final String checkNumber) {
        if ((description == null) || (checkNumber == null)) {
            return description;
        }

        if (description.equals("CHECK") && !checkNumber.isEmpty()) {
